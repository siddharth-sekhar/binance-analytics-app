import { useState } from 'react';
import axios from 'axios';

function DataIngestion({ apiBase, onSymbolsUpdate }) {
  const [wsSymbols, setWsSymbols] = useState('btcusdt,ethusdt');
  const [ingesting, setIngesting] = useState(false);
  const [message, setMessage] = useState('');
  const [ohlcSymbol, setOhlcSymbol] = useState('btcusdt');
  const [ohlcTimeframe, setOhlcTimeframe] = useState('1s');
  const [ohlcFile, setOhlcFile] = useState(null);
  const [uploading, setUploading] = useState(false);

  const handleStartWebSocket = async () => {
    if (!wsSymbols.trim()) {
  setMessage('Please enter at least one symbol');
      return;
    }

    setIngesting(true);
    setMessage('');
    
    try {
      const symbols = wsSymbols.split(',').map(s => s.trim()).filter(Boolean);
      const response = await axios.post(`${apiBase}/ingest/start`, {
        mode: 'ws',
        symbols
      });

      if (response.data.status === 'ok') {
        setMessage(`Successfully started WebSocket ingestion for: ${symbols.join(', ')}`);
        setTimeout(() => onSymbolsUpdate(), 2000); // Refresh symbols after 2 seconds
      } else {
        setMessage(`Error: ${response.data.msg}`);
      }
    } catch (error) {
      setMessage(`Error: ${error.message}`);
    } finally {
      setIngesting(false);
    }
  };

  return (
    <div className="ingestion-panel">
  <h2 style={{ marginBottom: '2rem', color: '#8b7ab8' }}>Data Ingestion</h2>

      {message && (
        <div 
          style={{
            padding: '1rem',
            marginBottom: '1.5rem',
            borderRadius: '8px',
            background: message.toLowerCase().startsWith('success') 
              ? 'rgba(38, 166, 154, 0.2)' 
              : 'rgba(239, 83, 80, 0.2)',
            border: `1px solid ${message.toLowerCase().startsWith('success') 
              ? 'rgba(38, 166, 154, 0.5)' 
              : 'rgba(239, 83, 80, 0.5)'}`
          }}
        >
          {message}
        </div>
      )}

      {/* WebSocket Ingestion */}
      <div style={{ marginBottom: '1rem' }}>
  <h3 style={{ marginBottom: '1rem', color: '#d0d0d0' }}>Live WebSocket Ingestion</h3>
        <p style={{ color: '#808080', marginBottom: '1rem', fontSize: '0.95rem' }}>
          Start real-time data ingestion from Binance WebSocket streams. Enter symbol pairs separated by commas.
        </p>
        
        <div className="control-group">
          <div className="control-item">
            <label>Symbol Pairs (comma-separated)</label>
            <input
              type="text"
              value={wsSymbols}
              onChange={(e) => setWsSymbols(e.target.value)}
              placeholder="e.g., btcusdt,ethusdt,bnbusdt"
              style={{ textTransform: 'lowercase' }}
            />
            <small style={{ color: '#aaa', fontSize: '0.85rem' }}>
              Examples: btcusdt, ethusdt, bnbusdt, adausdt, solusdt
            </small>
          </div>
        </div>

        <button
          className="button"
          onClick={handleStartWebSocket}
          disabled={ingesting}
          style={{ marginTop: '1rem' }}
        >
          {ingesting ? 'Starting...' : 'Start WebSocket Ingestion'}
        </button>
      </div>

      {/* OHLCV CSV Upload */}
      <div style={{ marginTop: '2rem' }}>
  <h3 style={{ marginBottom: '1rem', color: '#d0d0d0' }}>Upload OHLCV CSV</h3>
        <p style={{ color: '#808080', marginBottom: '1rem', fontSize: '0.95rem' }}>
          Upload precomputed OHLCV bars for a single symbol and timeframe. CSV must include columns: ts, open, high, low, close, volume.
        </p>

        <div className="control-group">
          <div className="control-item">
            <label>Symbol</label>
            <input
              type="text"
              value={ohlcSymbol}
              onChange={(e) => setOhlcSymbol(e.target.value.toLowerCase())}
              placeholder="e.g., btcusdt"
              style={{ textTransform: 'lowercase' }}
            />
          </div>

          <div className="control-item">
            <label>Timeframe</label>
            <select value={ohlcTimeframe} onChange={(e) => setOhlcTimeframe(e.target.value)}>
              <option value="1s">1 second</option>
              <option value="1min">1 minute</option>
              <option value="5min">5 minutes</option>
            </select>
          </div>

          <div className="control-item">
            <label>CSV File</label>
            <input
              type="file"
              accept=".csv,text/csv"
              onChange={(e) => setOhlcFile(e.target.files?.[0] || null)}
            />
            <small style={{ color: '#aaa', fontSize: '0.85rem' }}>
              Required headers: ts, open, high, low, close, volume
            </small>
          </div>

          <div className="control-item" style={{ justifyContent: 'flex-end' }}>
            <button
              className="button"
              style={{ marginTop: 'auto' }}
              disabled={uploading || !ohlcFile || !ohlcSymbol}
              onClick={async () => {
                if (!ohlcFile || !ohlcSymbol) return;
                setUploading(true);
                setMessage('');
                try {
                  const form = new FormData();
                  form.append('symbol', ohlcSymbol);
                  form.append('timeframe', ohlcTimeframe);
                  form.append('file', ohlcFile);
                  const resp = await axios.post(`${apiBase}/upload_ohlcv`, form, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                  });
                  if (resp.data.status === 'ok') {
                    setMessage(`Uploaded OHLCV for ${ohlcSymbol.toUpperCase()} @ ${ohlcTimeframe}`);
                    setTimeout(() => onSymbolsUpdate(), 1000);
                  } else {
                    setMessage(`Error: ${resp.data.msg || 'Upload failed'}`);
                  }
                } catch (err) {
                  setMessage(`Error: ${err.message}`);
                } finally {
                  setUploading(false);
                }
              }}
            >
              {uploading ? 'Uploading...' : 'Upload OHLCV CSV'}
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}

export default DataIngestion;
